...
internal class NullSafeKotlinJsonAdapter<T>(
    ...
    override fun toString() = "NullSafeKotlinJsonAdapter(${constructor.returnType})"
...
/**
 * 自定义KotlinJsonAdapter, 避免服务端应传的字段没有传导致的Required value xxx missing at ... 的问题
 *
 */
class NullSafeKotlinJsonAdapterFactory : JsonAdapter.Factory {
    override fun create(type: Type, annotations: MutableSet<out Annotation>, moshi: Moshi)
        ...
        for (property in rawTypeKotlin.memberProperties) {
            ...
            bindingsByName[property.name] = NullSafeKotlinJsonAdapter.Binding(name, adapter,
        ...
        return NullSafeKotlinJsonAdapter(constructor, bindings, options).nullSafe()
    }
}
